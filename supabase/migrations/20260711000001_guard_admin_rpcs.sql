-- Add admin guard to 3 RPCs missing the public.is_admin(auth.uid()) check
-- Requires: 20260711000000_create_is_admin_function

-- 1. get_top_active_users — already plpgsql, just add guard
...
-- 2. get_active_users_count — was LANGUAGE sql, convert to plpgsql to add guard
...
-- 3. get_unique_reports_user_count — was LANGUAGE sql, convert to plpgsql to add guard
