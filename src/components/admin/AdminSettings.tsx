...
export const AdminSettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedNotifs = localStorage.getItem('admin-notif-prefs');
    if (savedNotifs) setNotifs(JSON.parse(savedNotifs));

    const handleThemeSync = (e: any) => setTheme(e.detail);
    const handleSidebarSync = (e: any) => setSidebarCollapsed(e.detail);
    
    window.addEventListener('theme-sync', handleThemeSync);
    window.addEventListener('admin-sidebar-toggle', handleSidebarSync);
    
    return () => {
      window.removeEventListener('theme-sync', handleThemeSync);
      window.removeEventListener('admin-sidebar-toggle', handleSidebarSync);
    };
  }, []);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await logActivity('Profile Updated', 'user', profile?.id || null, `Admin profile name updated`);
      } catch (logErr) {
    ...
    try {
      ...
      try {
        await logActivity(
          ...
          'user',
          profile?.id || null,
          `Password changed successfully for account ${profile?.email}`
        );
      } catch (logErr) {
  ...
  const savePreferences = () => {
    localStorage.setItem('admin-notif-prefs', JSON.stringify(notifs));
    setToast({ message: 'Preferences synchronized', type: 'success' });
  };
  ...
  const handleLogoutAll = async () => {
    ...
    try {
      ...
      setToast({ message: 'Failed to terminate sessions', type: 'error' });
      setSaving(false);
    ...
    <div className="max-w-4xl mx-auto space-y-12">
        ...
        {showLogoutModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md bg-canvas rounded-[2.5rem] p-10 shadow-premium-2xl border border-hairline"
            >
              <div className="w-16 h-16 rounded-2xl bg-red-500/10 flex items-center justify-center mb-8">
                <LogOut className="w-8 h-8 text-red-500" />
              </div>
              <h3 className="text-2xl font-black tracking-tighter text-ink mb-4">Logout All Sessions?</h3>
              <p className="text-sm text-mute font-medium leading-relaxed mb-10">This will terminate every active session across all devices. You will need to login again on this device.</p>
              <div className="flex flex-col sm:flex-row gap-4">
                <button 
                  onClick={handleLogoutAll}
                  disabled={saving}
                  className="flex-1 h-14 bg-red-500 text-white rounded-pill font-black text-[11px] uppercase tracking-widest hover:bg-red-600 transition-all flex items-center justify-center gap-3"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
            ...
            <div className="space-y-8">
              ...
              {profile?.provider === 'google' ? (
                ...
                <form onSubmit={handleChangePassword} className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-mono font-black text-mute uppercase tracking-widest ml-4">Current Password</label>
                    <input 
                      type="password" 
          ...
          {/* PREFERENCES SECTION */}
          <section className="bg-canvas border border-hairline rounded-[2.5rem] p-8 sm:p-10 shadow-premium-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-500/[0.02] to-transparent pointer-events-none" />
            <div className="space-y-8">
              <div className="flex items-center gap-4 pb-6 border-b border-hairline">
                <div className="p-3 rounded-2xl bg-canvas-soft border border-hairline">
                ...
                <div>
                  <h3 className="text-lg font-black tracking-tight text-ink">System Preferences</h3>
                  <p className="text-xs text-mute font-medium">Customize your administrative environment.</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="text-[10px] font-mono font-black text-mute uppercase tracking-widest ml-4">Appearance</label>
                  <div className="flex bg-canvas-soft border border-hairline rounded-pill p-1">
                    <button 
                      onClick={() => toggleTheme('light')}
                      className={`flex-1 py-2 rounded-pill text-[10px] font-black uppercase tracking-widest transition-all ${theme === 'light' ? 'bg-canvas text-ink shadow-premium-sm' : 'text-mute hover:text-ink'}`}
                    >
                      Light
                    </button>
                    <button 
                      onClick={() => toggleTheme('dark')}
                      className={`flex-1 py-2 rounded-pill text-[10px] font-black uppercase tracking-widest transition-all ${theme === 'dark' ? 'bg-canvas text-ink shadow-premium-sm' : 'text-mute hover:text-ink'}`}
                    >
                      Dark
                    </button>
                  </div>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-mono font-black text-mute uppercase tracking-widest ml-4">Sidebar State</label>
                  <div className="flex bg-canvas-soft border border-hairline rounded-pill p-1">
                    <button 
                      onClick={() => toggleSidebar(false)}
                      className={`flex-1 py-2 rounded-pill text-[10px] font-black uppercase tracking-widest transition-all ${!sidebarCollapsed ? 'bg-canvas text-ink shadow-premium-sm' : 'text-mute hover:text-ink'}`}
                    >
                      Expanded
                    </button>
                    <button 
              ...
              <div className="space-y-6 pt-6 border-t border-hairline">
                <label className="text-[10px] font-mono font-black text-mute uppercase tracking-widest ml-4">Notification Preferences</label>
                <div className="space-y-4">
                  {[
                    { key: 'userReg', label: 'New User Registrations' },
                    { key: 'newReports', label: 'New Health Reports' },
                    { key: 'securityEvents', label: 'Security & Login Alerts' }
                  ].map((item) => (
                    <div key={item.key} className="flex items-center justify-between px-6 py-4 bg-canvas-soft border border-hairline rounded-2xl">
                      ...
                      <button 
                        ...
                        className={`w-10 h-5 rounded-full relative transition-colors ${notifs[item.key as keyof typeof notifs] ? 'bg-status-healthy' : 'bg-mute/20'}`}
                      >
                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${notifs[item.key as keyof typeof notifs] ? 'left-6' : 'left-1'}`} />
                      </button>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-4">
                <button 
                  ...
                  className="h-14 px-10 bg-canvas border border-hairline text-ink rounded-pill font-black text-[11px] uppercase tracking-[0.2em] shadow-premium-sm hover:bg-canvas-soft active:scale-95 transition-all flex items-center gap-3"
                >
          ...
          {/* DANGER ZONE */}
          <section className="bg-red-500/5 border border-red-500/10 rounded-[2.5rem] p-8 sm:p-10 shadow-premium-sm relative overflow-hidden">
            <div className="space-y-8">
              <div className="flex items-center gap-4 pb-6 border-b border-red-500/10">
                <div className="p-3 rounded-2xl bg-red-500/10 border border-red-500/20">
                  <Trash2 className="w-5 h-5 text-red-500" />
                </div>
                <div>
                  <h3 className="text-lg font-black tracking-tight text-red-600">Danger Zone</h3>
                  <p className="text-xs text-red-500/60 font-medium">Critical system actions with permanent effects.</p>
                </div>
              </div>

              <div className="flex items-center justify-between p-8 bg-canvas border border-red-500/20 rounded-3xl">
                <div>
                  <p className="text-sm font-black text-ink">Logout All Sessions</p>
                  <p className="text-xs text-mute font-medium opacity-60">Instantly terminate all active sessions globally.</p>
                </div>
                <button 
                  onClick={() => setShowLogoutModal(true)}
                  className="h-12 px-6 bg-red-500 text-white rounded-pill font-black text-[10px] uppercase tracking-widest hover:bg-red-600 transition-all shadow-premium-md"
                >
                  Terminate Sessions
                </button>
              </div>
        ...
        {/* Right Column: SECURITY SECTION & ACCOUNT INFORMATION */}
        <div className="space-y-8">
          ...
          {/* SECURITY SECTION Metadata */}
          <div className="bg-canvas border border-hairline rounded-[2.5rem] p-8 shadow-premium-sm space-y-8">
             <h4 className="text-[10px] font-mono font-black text-mute uppercase tracking-[0.3em]">Security Registry</h4>
             ...
             <div className="space-y-5">
               ...
               <div className="flex items-center gap-4">
                 ...
                 <div className="flex flex-col">
                   <span className="text-[9px] font-mono uppercase tracking-widest text-mute opacity-40">Account Created</span>
               ...
               <div className="flex items-center gap-4">
                 ...
                 <div className="flex flex-col">
                   <span className="text-[9px] font-mono uppercase tracking-widest text-mute opacity-40">Current Session</span>
                   <span className="text-[11px] font-black text-ink truncate max-w-[150px]">Secure TLS / Active</span>
                 </div>
               </div>
             </div>
             ...
             <div className="pt-6 border-t border-hairline">
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-ink/5 border border-ink/10">
                   ...
                   <span className="text-[9px] font-mono font-black uppercase tracking-tighter text-ink opacity-60">Registry Synchronized</span>
                </div>
             </div>
          </div>
        </div>
