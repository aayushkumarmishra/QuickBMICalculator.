    ...
    >
      <div className="max-w-[1180px] mx-auto px-5 w-full flex items-center h-full">
        ...
        <a href={isAdmin ? "/admin" : "/"} className="flex items-center gap-2.5 group z-50 shrink-0">
          ...
          <span className="font-bold tracking-[-0.03em] text-ink text-lg leading-none">QuickBMI</span>
        </a>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="absolute top-0 left-0 w-full h-[100dvh] bg-canvas pt-20 px-6 lg:hidden z-40 overflow-y-auto"
          >
            <div className="flex flex-col gap-6 pb-12">
              {primaryLinks.map((item) => (
                <a 
                  ...
                  className="text-xl font-semibold text-ink hover:text-mute transition-colors"
                >
                  {item.name}
                </a>
              ))}
              ...
              {!isAdmin && (
                <div className="flex flex-col gap-3 pl-4 border-l-2 border-hairline/60 my-1">
                  <span className="text-[10px] uppercase tracking-[0.25em] text-mute font-black mb-1">Tools</span>
                  {toolLinks.map((tool) => (
                    <a 
                      key={tool.name}
                      href={tool.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="text-base font-semibold text-mute hover:text-ink transition-colors"
                    >
                      {tool.name}
                    </a>
                  ))}
              ...
              <div className="pt-6 border-t border-hairline flex flex-col gap-4">
                {!user && (
                  <div className="grid grid-cols-2 gap-4">
                    <a 
                      href="/login" 
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="py-3 border border-hairline text-ink rounded-full text-center font-semibold text-sm hover:bg-canvas-soft transition-colors animate-fade-in"
                    >
                      Sign In
                    </a>
                    <a 
                      href="/signup" 
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="py-3 bg-ink text-canvas rounded-full text-center font-semibold text-sm hover:bg-ink-soft/90 transition-colors animate-fade-in"
                    >
                      Join Free
                    </a>
                  </div>
                ...
                {!isAdmin && (
                  <a 
                    href={calcHref} 
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="w-full py-3 bg-ink text-canvas rounded-full text-center font-semibold text-sm tracking-wide block hover:bg-ink-soft/90 transition-colors"
                  >
                    Calculate Now
                  </a>
                )}
